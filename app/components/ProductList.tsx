          ...
          {Products.map((product) => (
            <div onClick={() => router.push(`/product-detail/${product.slug}`)} className="group cursor-pointer">
              <div key={product.id} className="p-5">
                <div className="relative" onMouseOver={() => handleMouseOver(product.id)}
                  ...
                  <Image src={isHovered === product.id ? product.image.front : product.image.back} alt={product.name} className="rounded-2xl h-auto" />
